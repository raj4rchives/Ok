...
function makeWeeklyPDF() {
  ...
  const { jsPDF } = window.jspdf || {};
  if (!jsPDF) { alert('PDF library missing h!'); return; }
  const pdf = new jsPDF({ orientation:'landscape', unit:'mm', format:'a4' });
  ...
  const summary = [['TOTAL QUESTIONS', totals.phy, totals.chem, totals.math, totals.total],['LECTURES', '', '', '', totals.lec]];
  if (pdf.autoTable) pdf.autoTable({startY:38,head:[['TYPE','PHYSICS','CHEMISTRY','MATHEMATICS','TOTAL']],body:summary,theme:'grid'});
  let yy = pdf.lastAutoTable ? pdf.lastAutoTable.finalY + 10 : 48;
  pdf.setFontSize(10); pdf.text('Daily log', 14, yy); yy += 5;
  const body = days.map(d => [d.date, d.lec, d.phy, d.chem, d.math, d.total]);
  if (pdf.autoTable) pdf.autoTable({startY:yy,head:[['DATE','LEC','PHYSICS QUESTIONS','CHEMISTRY QUESTIONS','MATHEMATICS QUESTIONS','TOTAL QUESTIONS']],body,theme:'grid',styles:{fontSize:8},headStyles:{fontSize:8}});
  const footerY = pdf.lastAutoTable ? pdf.lastAutoTable.finalY + 8 : 190;
  pdf.setFont('helvetica','bold'); pdf.setFontSize(9);
  pdf.text(`Total lectures: ${totals.lec}   |   Total questions: ${totals.total}   |   Physics: ${totals.phy}   |   Chemistry: ${totals.chem}   |   Mathematics: ${totals.math}`, 14, footerY);
