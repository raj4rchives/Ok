...
function renderFocus(){
 ...
 $("focusLogs").innerHTML=logs.length?logs.slice(0,80).map(x=>`<div class="focus-log"><div><b>${esc(x.subject)} · ${esc(x.activity)}</b><small>${x.date}${x.note?" · "+esc(x.note):""}${x.questions?" · "+x.questions+" questions":""}</small></div><strong>${fmtMin(x.minutes)}</strong><button onclick="deleteFocus(${x.id})">×</button></div>`).join(""):`<div class="empty">No focus logs yet. Start your first session.</div>`;
 renderFocusHistory(logs);renderFocusChart();
}
function focusByDate(days=30){
 const map={};readFocus().forEach(x=>map[x.date]=(map[x.date]||0)+(+x.minutes||0));
...
function renderFocusHistory(logs){
 const box=$("focusDailyHistory");if(!box)return;const rows=focusByDate(30).reverse();
 const has=rows.some(x=>x.minutes>0);box.innerHTML=has?rows.map(x=>`<div class="daily-row"><span class="daily-date">${new Date(x.date+"T12:00:00").toLocaleDateString(undefined,{weekday:"short",day:"numeric",month:"short"})}</span><div class="daily-bar"><i style="width:${Math.min(100,(x.minutes/Math.max(60,Math.max(...rows.map(r=>r.minutes))))*100)}%"></i></div><b>${fmtMin(x.minutes)}</b></div>`).join(""):`<div class="empty">No focus history yet. Saved focus sessions will appear here date-wise.</div>`;
}
