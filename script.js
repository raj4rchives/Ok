...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 setInterval(updateMiniDashboard,1000);updateMiniDashboard();renderFocus();renderTodos();restoreTimerState();
});
...
function renderFocus(){
 ...
 $("focusLogs").innerHTML=logs.length?logs.slice(0,80).map(x=>`<div class="focus-log"><div><b>${esc(x.subject)} · ${esc(x.activity)}</b><small>${focusDate(x)}${x.note?" · "+esc(x.note):""}${x.questions?" · "+x.questions+" questions":""}</small></div><strong>${fmtMin(x.minutes)}</strong><button onclick="deleteFocus(${x.id})">×</button></div>`).join(""):`<div class="empty">No focus logs yet. Start your first session.</div>`;
 renderFocusHistory(logs);renderFocusChart();
}
function focusDate(x){
  if(x && x.date && /^\d{4}-\d{2}-\d{2}$/.test(x.date)) return x.date;
  if(x && x.timestamp){ const d=new Date(x.timestamp); if(!isNaN(d)) return iso(d); }
  return iso(new Date());
}
function focusByDate(days=30){
 const map={};readFocus().forEach(x=>{const d=focusDate(x);map[d]=(map[d]||0)+(+x.minutes||0);});
 const out=[],now=new Date();for(let i=days-1;i>=0;i--){let d=new Date(now);d.setHours(12,0,0,0);d.setDate(d.getDate()-i);let key=iso(d);out.push({date:key,minutes:map[key]||0});}return out;
}
function renderFocusHistory(logs){
 const box=$("focusDailyHistory"); if(!box)return;
 const rows=focusByDate(30).filter(x=>x.minutes>0).sort((a,b)=>b.date.localeCompare(a.date));
 if(!rows.length){box.innerHTML=`<div class="empty">No focus history yet. Save a focus session and it will appear here date-wise.</div>`;return;}
 const max=Math.max(60,...rows.map(r=>r.minutes));
 box.innerHTML=rows.map(x=>`<div class="daily-row" data-date="${x.date}"><span class="daily-date"><b>${new Date(x.date+"T12:00:00").toLocaleDateString(undefined,{weekday:"short",day:"numeric",month:"short"})}</b><small>${x.date}</small></span><div class="daily-bar"><i style="width:${Math.min(100,(x.minutes/max)*100)}%"></i></div><b>${fmtMin(x.minutes)}</b></div>`).join("");
}
function renderFocusChart(){
 const box=$("focusChart");if(!box)return;const rows=focusByDate(14),max=Math.max(60,...rows.map(x=>x.minutes)),total=rows.reduce((s,x)=>s+x.minutes,0);
 $("focusDashboardTotal").textContent=fmtMin(total);
 box.innerHTML=rows.map(x=>{const h=Math.max(3,Math.round(x.minutes/max*100));const label=new Date(x.date+"T12:00:00").toLocaleDateString(undefined,{day:"numeric",month:"short"});return `<div class="chart-col" title="${x.date}: ${fmtMin(x.minutes)}"><div class="chart-value">${x.minutes?fmtMin(x.minutes):""}</div><div class="chart-bar-wrap"><i style="height:${h}%"></i></div><small>${label}</small></div>`}).join("");
}
...
function saveTimer(){
  ...
  const now=Date.now(), mins=Math.round(elapsed/60),a=readFocus();
  a.push({id:now,timestamp:now,date:iso(new Date(now)),minutes:mins,questions:0,subject:$("focusSubject").value,activity:$("focusActivity").value,note:$("focusNote").value});
  writeFocus(a);
  ...
  renderFocus();updateMiniDashboard();
  showFocusToast(`Focus saved — ${fmtMin(mins)}`);
}

function showFocusToast(message){
 let t=$("focusToast");
 if(!t){t=document.createElement("div");t.id="focusToast";t.className="focus-toast";document.body.appendChild(t);}
 t.textContent="✓ "+message;t.classList.add("show");clearTimeout(window.__focusToast);window.__focusToast=setTimeout(()=>t.classList.remove("show"),2200);
}
